...
app.post('/createcoll', async (req, res) => {
  ...
  await database.createCollection(name, descr, category, id, inputs[0], inputs[1], inputs[2], linkToImg);
  res.json({ message: 'ok' });
});
...
app.post('/changeItem', async (req, res) => {
  const name = req.body.name;
  const tag = req.body.tagsValues;
  const value1 = req.body.field1Value;
  const value2 = req.body.field2Value;
  const value3 = req.body.field3Value;
  const id = req.body.selected;
  const tags = tag.map((el) => el.startsWith('#') ? el : `#${el}`).join(' ');
  await database.changeItem(id, name, tags, value1, value2, value3);
  res.end();
});
